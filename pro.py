...
img3 = Image.open(empty).convert('RGB')
...
def 당산():
    ...
    tab1,tab2,tab3,tab4,tab5=st.tabs(['역사지도','승/하차 예상인원','개찰구제어','에스컬레이터제어','대시보드'])
    with tab1:
        st.text('당산')
        st.image('당산.png')
    with tab2:
...
def 여의도():
    ...
    df2.reset_index(drop=True,inplace=True)
    df3.reset_index(drop=True,inplace=True)
    tab1,tab2,tab3,tab4=st.tabs(['역사지도','승/하차 예상인원','개찰구제어','대시보드'])
    with tab1:
        st.text('여의도')
        st.image('여의도.png')
    with tab2:
    ...
    with tab4:
        if st.button('예상 혼잡도 대시보드'):
            webbrowser.open("https://public.tableau.com/app/profile/my8189/viz/_16725866122850/1_1?publish=yes")
